...
const makeSut = () => {
    ...
    const sut = new RecommendationByData(getTableFieldsClient, client);
...
describe('recommendation - search by data', () => {
    ...
    test('should call the expected url', async () => {
        ...
        expect(client.urlCalled).toBe(`recommendation/data/${dummyCollectionName}?top_k=${5}`);
    });
    ...
    test('should call the expected url with custom topK', async () => {
        ...
        expect(client.urlCalled).toBe(`recommendation/data/${dummyCollectionName}?top_k=${20}`);
    });
